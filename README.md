...
The scFFPE-seq data established a cellular reference of 17 annotated cell types including two molecularly distinct forms of ductal carcinoma in situ (DCIS #1 and DCIS #2), invasive tumor cells, two myoepithelial subtypes, immune cells, stromal cells and endothelial cells.
...
Both formats store the same gene x cell count matrix. The HDF5 file is faster to load and preferred for large datasets. Raw data is also available from the 10x Genomics preview dataset page linked in the Data Availability section.
...
These three filters follow directly from the Methods section of the paper and each has a clear biological justification. High mitochondrial fraction indicates a cell where cytoplasmic RNA has leaked out while mitochondria are retained, which is a hallmark of cell damage. Too few detected genes means the GEM well likely captured an empty droplet or debris rather than a real cell. A single gene dominating more than 20% of counts suggests a technical artifact rather than genuine biology.
...
- **Invasive Tumor** is characterized by strong **FASN** and **CDH2** expression, complete absence of myoepithelial markers, and the disappearance of **MMP12** from macrophages, a pattern consistent with active invasion and remodeling of the surrounding stroma.
...
The AnnData file obtained from the scFFPE-seq pipeline contains the full whole-transcriptome scFFPE-seq matrix (~27,000 cells x 18,536 genes) with cell-type annotations. No manual downloading or Drive mounting is required as the notebook handles the download automatically.
...
The dot plot displays 10 cell types against 7 gene-category columns. Each dot's size encodes the fraction of cells in that group expressing the gene, and the color encodes scaled mean expression (dark red = high, dark blue = low).
...
**MMP12** is essentially absent across all three tumor subtypes, which is expected since MMP12 is a macrophage-secreted metalloproteinase and not a tumor cell marker. Its near-zero expression here confirms that the DGE is cleanly reflecting cell-intrinsic tumor programs rather than contaminating macrophage signal. The paper's finding that MMP12 is absent in the Invasive ROI refers to the macrophage population within that region, which is appropriately captured in the dot plot (Macrophages rows) rather than in the tumor-subtype DGE.
...
The raw transcript-level file was excluded since it runs several gigabytes in size and wasn't needed to reproduce Figure 3b.
